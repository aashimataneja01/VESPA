...
def query_reviews_by_embedding(embedding_vector):
    query = {
        'hits': 5,
...
doc_id= "3rPt0LxF7rgmEUrznoH22w"
emb = get_embedding("doc_id")
if emb:
    results = query_reviews_by_embedding(emb["fields"]["embedding"])
    df = display_hits_as_df(results, ["doc_id", "title", "text"])
    print("\nQuery Results by Embeddding")
    print(df.head())
else:
    print(f"Embedding for {doc_id} not found.")    
